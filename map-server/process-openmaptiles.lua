...
function node_function()
	...
	-- Write 'railway_station' (railway/public_transport stations as point)
	local railway = Find("railway")
	local public_transport = Find("public_transport")
	if railway == "station" or railway == "halt" or railway == "tram_stop" or public_transport == "station" then
		Layer("railway_station", false)
		SetNameAttributes()
		if railway ~= "" then
			Attribute("class", railway)
		else
			Attribute("class", "station")
		end
		local station = Find("station")
		if station ~= "" then Attribute("subclass", station) end
	end
	...
	if place ~= "" then
		-- Demote 中国市辖区: OSM 常把 "萧山区/朝阳区/蜀山区" 等标成 place=city
		-- 检测名字末尾的 "区" (UTF-8: E5 8C BA, 3 bytes), 降级为 suburb
		if place == "city" then
			local _name = Find("name")
			if #_name >= 3 and _name:sub(-3) == "区" then
				place = "suburb"
			end
		end
...
function way_function()
	...
	-- Administrative boundaries (only country/state/province for travel maps)
	-- https://openmaptiles.org/schema/#boundary
	if isBoundary and admin_level <= 6 and not (Find("maritime")=="yes") then
		local mz = 0
	...
	-- Roads ('transportation' and 'transportation_name') -- only major roads
	if highway ~= "" then
		local h = highway
		local subclass = nil
		...
		if majorRoadValues[h]   then minzoom = 4       -- motorway/trunk/primary
		elseif h == "trunk"     then minzoom = 5
		elseif h == "primary"   then minzoom = 7
		elseif z9RoadValues[h]  then minzoom = 9       -- secondary, motorway_link, trunk_link
		elseif z10RoadValues[h] then minzoom = 10      -- primary_link, secondary_link
		elseif z11RoadValues[h] then minzoom = 11      -- tertiary
		elseif z12MinorRoadValues[h] or z12OtherRoadValues[h] then minzoom = 12
		elseif z13RoadValues[h] then minzoom = 13      -- service, track
		elseif pathValues[h]    then minzoom = 14      -- footway, cycleway
		end
		...
		if minzoom <= 14 then
			write_to_transportation_layer(minzoom, h, subclass, ramp, service, false, true, false)

			if not is_closed and (HasNames() or Holds("ref")) then
				if h == "motorway" then minzoom = 7
				elseif h == "trunk" then minzoom = 8
				elseif h == "primary" then minzoom = 10
				elseif h == "secondary" then minzoom = 12
				elseif h == "tertiary" then minzoom = 13
				else minzoom = 14
				end
	...
	-- Railways ('transportation') -- only main rail at z<=10
	if railway~="" then
	...
	-- Railway/public_transport stations as area centroid
	if railway == "station" or railway == "halt" or public_transport == "station" then
		LayerAsCentroid("railway_station")
		SetNameAttributes()
		if railway ~= "" then
			Attribute("class", railway)
		else
	...
	if aeroway~="" and aeroway ~= "aerodrome" then
		Layer("aeroway", is_closed)
		Attribute("class", aeroway)
		Attribute("ref", Find("ref"))
	end

	-- 'aerodrome_label' (airport polygons -> centroid)
	if aeroway=="aerodrome" then
	...
	-- Set 'water' (lakes/reservoirs/large rivers as polygons)
	...
	-- 'building' (closed way with building tag)
	local building = Find("building")
	if (building ~= "" and building ~= "no") and is_closed then
		Layer("building", true)
		SetMinZoomByAreaWithLimit(13)
