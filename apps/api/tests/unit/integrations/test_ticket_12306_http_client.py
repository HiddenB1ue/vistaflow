...
def _build_client(
    ...
    *,
    cookie_pool: StubCookiePool | None = None,
) -> tuple[HttpTicketClient, StubCookiePool]:
    pool = cookie_pool or StubCookiePool()
    client = HttpTicketClient(
        cookie_pool=pool,  # type: ignore[arg-type]
        max_concurrency=2,
...
def test_fetch_leg_returns_rows_on_success() -> None:
    ...
    client, pool = _build_client(httpx.MockTransport(handler))
    rows = asyncio.run(
...
def test_fetch_leg_treats_persistent_redirect_as_rate_limit() -> None:
    calls = {"count": 0}
    ...
    client, pool = _build_client(httpx.MockTransport(handler))
    with pytest.raises(TicketHttpFailure, match="redirect 302 persisted"):
        asyncio.run(
            client.fetch_leg("2026-04-28", "Beijing", "Shanghai", "BJP", "SHH")
        )
    # 1 initial + 3 backoff retries = 4 total requests
    assert calls["count"] == 4
    # Only invalidated after all retries exhausted
    assert pool.invalidated == 1


def test_fetch_leg_redirect_recovers_after_backoff() -> None:
    calls = {"count": 0}
    ...
    # Cookie was NOT invalidated since it recovered via backoff
    assert pool.invalidated == 0
