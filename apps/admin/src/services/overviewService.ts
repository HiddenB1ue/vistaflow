...
import { MOCK_KPI_STATS, MOCK_SPARKLINE_DATA } from './mock/overview.mock';
...
export async function fetchSparklineData(days: number = 7): Promise<SparklineData> {
  if (USE_MOCK) {
    return {
      values: MOCK_SPARKLINE_DATA.values.slice(-days),
      labels: MOCK_SPARKLINE_DATA.labels.slice(-days),
    };
  }
